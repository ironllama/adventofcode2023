...
module sorts
  ...
  private :: binary_search, merge_arrays, heapify, heapify_nr, &
       ...
       radixsort2, radixsort_uint64_r, radixsort_uint64, &
       radixsort_opt2, radixsort_opt3
...
contains
  ...
  subroutine interheap_sift(array,root,mask,offset)
   ...
   subroutine radixsort_uint64(array)
    integer(i64), intent(inout), target::array(:)
    integer(i64), allocatable, target::work(:)
    integer(i64)::mask
    integer(i64), pointer::ptr_in(:),ptr_out(:),ptr_tmp(:)
    integer::i,tmp,offset,shift,count(0:256)
    ...
    do while (shift.lt.64)
       count=0
       do i=1,size(array)
          tmp=shiftr(iand(ptr_in(i),mask),shift)+1
       ...
       enddo
       do i=1,size(array)
          tmp=shiftr(iand(ptr_in(i),mask),shift)
          offset=count(tmp)+1
          count(tmp)=offset
          ptr_out(offset)=ptr_in(i)
       enddo
    ...
    enddo
    deallocate(work)
  end subroutine radixsort_uint64

  subroutine radixsort_uint64_r(array)
    integer(i64), intent(inout), target::array(:)
    integer(i64), allocatable, target::work(:)
    integer(i64)::mask
    integer(i64), pointer::ptr_in(:),ptr_out(:),ptr_tmp(:)
    ...
    enddo
    deallocate(work)
  end subroutine radixsort_uint64_r

  subroutine radixsort2(array)
    use iso_c_binding
    real(prec), intent(inout), target, contiguous::array(:)
    integer :: last,left,right
    integer(i64)::temp
    integer(i64), pointer::dummy(:)

    last=size(array)
    if (last.le.1) return

    call c_f_pointer(c_loc(array),dummy,[last])
    left=1
    right=last
    do
       do while(btest(dummy(left),63))
          left=left+1
          if (left.gt.last) exit
       enddo
       do while(.not.btest(dummy(right),63))
          right=right-1
          if (right.lt.1) exit
       enddo
       if (left.ge.right) exit
    ...
    if (left.gt.1) then
       call radixsort_uint64_r(dummy(:left-1))
    endif

    if (right.lt.last) then
       call radixsort_uint64(dummy(left:))
    endif

  end subroutine radixsort2

  ! wrapper to hide target specifier from calling function
  subroutine radixsort(array)
    real(prec), intent(inout)::array(:)
    call radixsort2(array)
  end subroutine radixsort

  subroutine radixsort_opt2(array)
    integer(i64), intent(inout), target, contiguous::array(:)
    integer(i64), allocatable, target::work(:)
    integer(i64)::tmp64
    integer(i64), pointer::ptr_in(:),ptr_out(:),ptr_tmp(:)
    integer::i,j,tmp,offset,shift,counts(0:255),off(0:255)
    real(prec)::rdummy
    ...
    counts=0
    do i=1,size(array)
       tmp=iand(int(ptr_in(i)),255)
       counts(tmp)=counts(tmp)+1
    enddo

    do while (shift.lt.56)
       off(0)=0
       tmp=counts(0)
       off(1)=tmp
       do i=2,255
       ...
       counts=0
       do i=1,size(array)
          tmp=iand(int(shiftr(ptr_in(i),shift)),65535)
          counts(shiftr(tmp,8))=counts(shiftr(tmp,8))+1
          tmp=iand(tmp,255)
          offset=off(tmp)+1
          off(tmp)=offset
          ptr_out(offset)=ptr_in(i)
    ...
    if (offset.gt.0) then
       do i=0,offset/2
          tmp64=ptr_out(i+1)
          ptr_out(i+1)=ptr_out(offset+1-i)
          ptr_out(offset+1-i)=tmp64
       enddo
    endif
    deallocate(work)
  end subroutine radixsort_opt2

  ! wrapper to change data type
  subroutine radixsort_opt3(array)
    use iso_c_binding
    real(prec), intent(inout), target, contiguous::array(:)
    integer :: last
    integer(i64), pointer::dummy(:)
    ...
    last=size(array)
    call c_f_pointer(c_loc(array),dummy,[last])
    call radixsort_opt2(dummy)
  end subroutine radixsort_opt3

  ! wrapper to hide target specifier from calling function
  subroutine radixsort_opt(array)
    real(prec), intent(inout)::array(:)
    call radixsort_opt3(array)
  end subroutine radixsort_opt
